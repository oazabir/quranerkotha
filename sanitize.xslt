...
<xsl:stylesheet version="1.0" 
  ...
  >
	...
	<xsl:template match="script|meta|footer|div[@id='fb-root']|link|div[@class='awac-wrapper']|div[@class='printfriendly']|div[@id='comments']|div[@id='widget-area']|div[@class='post-thumbnail']|hr">
	</xsl:template>
			...
			<xsl:variable name="output">
				<xsl:choose>
					<xsl:when test="starts-with($href,'https://quranerkotha.com/')">
						...
						<xsl:value-of select="substring-after($href,'https://quranerkotha.com')" />
					</xsl:when>
					<xsl:otherwise>
	...
	<!-- make img src attributes relative for both http and https -->
	<xsl:template match="img/@src[starts-with(.,'http://quranerkotha.com/')]">
		<xsl:attribute name="src">
			<xsl:value-of select="substring-after(.,'http://quranerkotha.com')"/>
		</xsl:attribute>
	</xsl:template>
	<xsl:template match="img/@src[starts-with(.,'https://quranerkotha.com/')]">
		<xsl:attribute name="src">
			<xsl:value-of select="substring-after(.,'https://quranerkotha.com')"/>
		</xsl:attribute>
	...
	<xsl:template match="head">
		<head>
			...
			<style>div.entry-content p { text-align: justify; }</style>
